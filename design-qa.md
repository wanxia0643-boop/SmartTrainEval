...
- Implementation screenshot path: `E:/SmartTrainEval/.codex-run/report-screen-datav.png`
...
- State: live dashboard with geoJson map, Three.js WebGL scene, CSS2D labels, CSS3D cards, ECharts panels, and digital-human fallback visible.
...
- Image quality and asset fidelity: center uses rendered 3D geoJson geometry, not a static screenshot. Digital-human area uses the existing educator visual as fallback; it switches to MoFa XingYun iframe when `VITE_MOFA_XINGYUN_EMBED_URL` is configured.
...
- Provide the real MoFa XingYun embed URL or SDK details to replace the fallback digital-human card.
